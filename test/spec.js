...
describe('Connect resource pipeline middleware', function () {
    ...
    describe('when processing content', function () {
        ...
        describe('using a pipeline', function () {
            ...
            it('should optionally cache the response body', function (done) {
                var url = '/test/fixtures/file.html';
                var newContent = 'This information has been changed.';
                var pipeline = sinon.spy(function (files) {
                    return files.pipe(replaceContent(newContent));
                });
                var middleware = resourcePipeline([{url: url, cache: true, pipeline: pipeline}]);
                // First request should cache response
                testMiddleware(url, middleware)
                    .verify(function () {}, function (err) {
                        if (err) {
                            return done(err);
                        }
                        ...
                        testMiddleware(url, middleware)
                            .verify(function (result) {
                                expect(result.content).to.equal(newContent);
                                expect(pipeline).to.have.been.calledOnce; // jshint ignore:line
                            }, done);
                    });
            });

            it('should allow cached content to be cleared', function (done) {
                var url = '/test/fixtures/file.html';
                var newContent = 'This information has been changed.';
                var pipeline = sinon.spy(function (files) {
                    return files.pipe(replaceContent(newContent));
                });
                var middleware = resourcePipeline([{url: url, cache: 'cacheId', pipeline: pipeline}]);
                // First request should cache response
                testMiddleware(url, middleware)
                    .verify(function () {}, function (err) {
                        if (err) {
                        ...
                        middleware.clear('cacheId');
                        // Pipeline should be hit again
                        testMiddleware(url, middleware)
                            .verify(function (result) {
                                expect(result.content).to.equal(newContent);
                                expect(pipeline).to.have.been.calledTwice; // jshint ignore:line
                            }, done);
                    });
            });
        });
